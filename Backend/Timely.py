...
try:
    ...
    fire_creds = json.loads('./vaulted-bivouac-445005-j5-98c05336b0e9.json')
    
    print("Firebase token loaded")
...
async def oauth_callback(request: fast_request):
    try:
        ...
        if not code:
            raise HTTPException(
                status_code=400, detail="Authorization code missing")
        ...
        asyncio.create_task(first_signin(
                        ...
                        match position:
                            ...
                            case "CHATTING":
                                if user_message == "/cancel":
                                    ...
                                    prev_msgs = chat_history_retriever(
                                        db, chat_id)
                                    await chat_handler(
                                        db,
                                        session,
